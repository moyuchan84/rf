...
import type { SelectedNode, ProcessPlan, BeolOption, Product } from '../types';
import { ChevronRight, ChevronDown, FolderTree, FileSpreadsheet, Box, Search, X } from 'lucide-react';
import { cn } from '@/shared/utils/cn';
import { useMasterData } from '../hooks/useMasterData';

const HierarchyExplorer: React.FC = () => {
  const { processPlans, selectedNode, setSelectedNode, setIsFormOpen, loading } = useMasterData();
  ...
  const filteredHierarchy = useMemo(() => {
    if (!searchTerm.trim()) return processPlans;
    const term = searchTerm.toLowerCase();

    return processPlans.map((plan: ProcessPlan) => {
    ...
    <div className="flex flex-col h-full space-y-4">
      ...
      <div className="relative group">
        ...
        <input 
          ...
          className="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl py-2 pl-9 pr-9 text-[11px] font-bold text-slate-900 dark:text-white placeholder:text-slate-400 outline-none focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/5 transition-all shadow-sm"
        />
        {searchTerm && (
          <button 
      ...
      <div className="flex-1 overflow-y-auto space-y-2 pr-2 custom-scrollbar">
        {filteredHierarchy.length === 0 && !loading ? (
          <div className="flex flex-col items-center justify-center h-32 text-slate-300 gap-2">
            ...
            <span className="text-[10px] font-black uppercase tracking-widest opacity-40 italic">No matches</span>
          </div>
        ) : (
          filteredHierarchy.map((pp) => (
            <div key={pp.id} className="space-y-1">
              ...
              >
                ...
                >
                  {expandedNodes[`plan-${pp.id}`] || searchTerm ? <ChevronDown className="w-3.5 h-3.5" /> : <ChevronRight className="w-3.5 h-3.5" />}
                </button>
              ...
              {(expandedNodes[`plan-${pp.id}`] || searchTerm) && (
                  ...
                  {(pp.beolOptions || []).map((bo) => (
                    <div key={bo.id} className="space-y-1">
                      ...
                      >
                        ...
                        >
                          {expandedNodes[`option-${bo.id}`] || searchTerm ? <ChevronDown className="w-3.5 h-3.5" /> : <ChevronRight className="w-3.5 h-3.5" />}
                        </button>
                      ...
                      {(expandedNodes[`option-${bo.id}`] || searchTerm) && (
