...
export const KeyTableSetupBucketSelector: React.FC = () => {
  const { 
    availableKeys,
    selectedTables, 
    addTable, 
  ...
  // Group available keys by photoCategory -> tableName
  ...
  // Group selected keys by photoCategory
  ...
  const PhotoKeyItem = ({ keyData, isSelected, onAction, showDetails = true }: { 
    ...
    <div className={`group flex items-center justify-between p-2 rounded-md transition-all ${
      isSelected 
        ? 'bg-emerald-50/50 dark:bg-emerald-900/10 border border-emerald-100 dark:border-emerald-800' 
  ...
  return (
    <div className="grid grid-cols-2 gap-8 h-[600px]">
      ...
      <div className="flex flex-col gap-4 border-r border-slate-100 dark:border-slate-800 pr-4 min-h-0 overflow-hidden">
          ...
          <div className="flex items-center gap-3">
            {availableKeys.length > 0 && (
              <button 
                ...
                className="flex items-center gap-1.5 text-[9px] font-black text-emerald-600 dark:text-emerald-400 hover:bg-emerald-50 dark:hover:bg-emerald-900/20 px-2 py-1 rounded transition-all"
              >
        ...
        <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar space-y-6 min-h-0">
          {availableKeys.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center gap-3 text-slate-300">
              ...
              <p className="text-[10px] font-black uppercase tracking-widest italic opacity-40">No data for this product</p>
            </div>
          ) : (
                  ...
                  {Object.entries(tables).map(([tableName, keys]) => {
                    ...
                    return (
                      <div key={tableName} className="border border-slate-50 dark:border-slate-800/50 rounded-md overflow-hidden bg-slate-50/30 dark:bg-slate-900/20">
                        ...
                        {isExpanded && (
                          <div className="px-2 pb-2 pt-1 border-t border-slate-100 dark:border-slate-800 bg-white/50 dark:bg-black/20 divide-y divide-slate-50 dark:divide-slate-800/50 max-h-[280px] overflow-y-auto custom-scrollbar">
                            {keys.map(key => (
                              <PhotoKeyItem 
                                key={key.id}
                                keyData={key}
      ...
      <div className="flex flex-col gap-4 min-h-0 overflow-hidden">
        <div className="flex items-center justify-between pb-2 border-b border-emerald-100 dark:border-emerald-900/30 shrink-0">
          <div className="flex items-center gap-2">
            <div className="w-6 h-6 rounded bg-emerald-600 flex items-center justify-center text-white shadow-lg shadow-emerald-600/20">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-[9px] font-black text-white bg-emerald-600 px-2 py-0.5 rounded-full shadow-sm">
                  ...
                  {keys.map(key => (
                    <div key={key.id} className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-md p-2 hover:shadow-sm transition-shadow">
                      <div className="flex justify-between items-start mb-1">
                        <span className="text-[10px] font-black text-emerald-600 dark:text-emerald-400 uppercase truncate pr-4">
                          {key.tableName}
                        </span>
                        <button 
        ...
        {selectedTables.length > 0 && (
          <div className="p-3 bg-emerald-50/50 dark:bg-emerald-900/10 border border-emerald-100/50 dark:border-emerald-800/50 rounded-lg flex gap-3">
            <Info className="w-4 h-4 text-emerald-500 shrink-0" />
            <p className="text-[8px] font-bold text-emerald-700/70 dark:text-emerald-400/70 leading-relaxed uppercase tracking-tight">
              Review setup selections. These tables define the key configuration for this request.
            </p>
          </div>
        )}
      </div>
