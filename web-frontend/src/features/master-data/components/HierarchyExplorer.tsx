    ...
    <div className="space-y-3">
      {processPlans.map((pp) => (
        <div key={pp.id} className="space-y-1">
          <div
            className={cn(
              'group flex items-center gap-3 p-3.5 rounded-2xl cursor-pointer transition-all border border-transparent shadow-sm',
          ...
          >
            ...
            <FolderTree className={cn("w-5 h-5 transition-colors", selectedNode?.type === 'plan' && selectedNode?.id === pp.id ? "text-white" : "text-indigo-500 dark:text-indigo-400")} />
            <span className="text-sm font-black truncate flex-1 uppercase tracking-wider transition-colors">{pp.designRule}</span>
            <button
              onClick={(e) => {
                e.stopPropagation();
              ...
              className={cn(
                "opacity-0 group-hover:opacity-100 w-7 h-7 rounded-xl flex items-center justify-center text-lg font-light transition-all active:scale-90 shadow-lg",
                selectedNode?.type === 'plan' && selectedNode?.id === pp.id 
          ...
          {expandedNodes[`plan-${pp.id}`] && (
            <div className="ml-6 pl-5 border-l border-slate-200 dark:border-slate-800 space-y-2 py-2 transition-colors">
              {pp.beolOptions.map((bo) => (
                <div key={bo.id} className="space-y-1">
                  <div
                    className={cn(
                      'group flex items-center gap-3 p-3 rounded-2xl cursor-pointer transition-all border border-transparent shadow-sm',
                  ...
                  >
                    ...
                    <FileSpreadsheet className={cn("w-4 h-4 transition-colors", selectedNode?.type === 'option' && selectedNode?.id === bo.id ? "text-white" : "text-blue-500 dark:text-blue-500/50")} />
                    <span className="text-xs font-black truncate flex-1 uppercase tracking-widest transition-colors">{bo.optionName}</span>
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                      ...
                      className={cn(
                        "opacity-0 group-hover:opacity-100 w-6 h-6 rounded-lg flex items-center justify-center text-sm font-light shadow-lg active:scale-90 transition-all",
                        selectedNode?.type === 'option' && selectedNode?.id === bo.id 
                  ...
                  {expandedNodes[`option-${bo.id}`] && (
                    <div className="ml-5 pl-5 border-l border-slate-200 dark:border-blue-900/30 space-y-1 py-1 transition-colors">
                      {bo.products.map((prod) => (
                        <div
                          key={prod.id}
                          className={cn(
                            'group flex items-center gap-3 p-2.5 rounded-xl cursor-pointer transition-all border border-transparent shadow-sm',
                            selectedNode?.type === 'product' && selectedNode?.id === prod.id
                              ? 'bg-emerald-600 text-white shadow-lg shadow-emerald-600/20 border-emerald-500/30'
                        ...
                        >
                          <Box className={cn("w-4 h-4 transition-colors", selectedNode?.type === 'product' && selectedNode?.id === prod.id ? "text-white" : "text-emerald-500 dark:text-emerald-500/30")} />
                          <div className="flex-1 min-w-0">
                            <div className="text-[10px] font-black uppercase tracking-[0.2em] transition-colors">{prod.partId}</div>
                            <div className="text-[9px] font-bold opacity-60 truncate uppercase transition-opacity">{prod.productName}</div>
                          </div>
                        </div>
                      ))}
                    </div>
